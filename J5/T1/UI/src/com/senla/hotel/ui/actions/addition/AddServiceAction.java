...
public class AddServiceAction implements IAction {
	...
	public void execute() {
		...
		try {
			Facade.getInstance().addService(new Service(IDGenerator.createServiceID(), Integer.parseInt(params[0].trim()),
					params[1].trim(), DateCreator.parseString(params[2].trim())));
