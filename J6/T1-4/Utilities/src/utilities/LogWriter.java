...
public class LogWriter {
	...
	private LogWriter() {
		...
		try {
			log.addHandler(
					new FileHandler(HotelProperties.getInstance().getProperty(PropertyNames.LOG_FILENAME.toString())));
		} catch (SecurityException | NullException e) {
			Printer.println(e.getMessage());
		} catch (IOException e) {
			Printer.println(Messages.NO_LOG_FILE_FOUND.toString());
		}
	}
